...
usps.update({
    'data': - usps.data,
    'noisy': - skimage.util.random_noise(usps.data, mode='gaussian', var=0.5 ** 2, seed=123),
...
for number, index in zip(vals, locs):
    ...
    image = np.array(usps.noisy[index]).reshape((16, 16))
    plt.imshow(image, cmap='gray', interpolation='none')
    plt.axis('off')
plt.tight_layout()
plt.show()
...
for number in range(10):
    ...
    denoised = kPCA(usps.data[train_idx], usps.noisy[test_idx]).obtain_preimages(20, 0.5)

    for i in range(10):
        plt.subplot(5, 4, 2 * i + 1)
        plt.imshow(usps.noisy[test_idx[i]].reshape((16, 16)), cmap='gray', interpolation='none')
...
for i, n_features in enumerate(range(2, 21, 2)):
    denoised = kPCA(usps.data[train_idx], usps.noisy[test_idx]).obtain_preimages(n_features, 0.5)
    plt.subplot(5, 4, 2 * i + 1)
    plt.imshow(usps.noisy[test_idx[0]].reshape((16, 16)), cmap='gray', interpolation='none')
